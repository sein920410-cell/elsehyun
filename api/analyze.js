...
━━ 이름 짓는 법 ━━
라벨/브랜드가 보이면 → 한국어 통용명 + 제품종류
  예: RYO → "려 샴푸" / ILLIYOON → "일리윤 바디로션" / Bébéen → "베베앙 물티슈"
  예: 만능클리너 → "만능클리너" / OL소화기 → "소화기" / Febreze → "페브리즈 섬유탈취제"

라벨 없으면 → 기능/용도 이름 (형태 묘사 금지)
  ✅ 옳은 예: "밀대", "우산", "소화기", "수납 바구니", "노트", "볼펜 묶음", "쿨링 스프레이", "드라이샴푸"
  ❌ 틀린 예: "긴 손잡이 도구", "직사각형 물체", "손잡이 도구", "플라스틱 용기", "검정 물건"

라벨에 제품명이 크게 쓰여 있으면 그대로 읽기
  예: 캔에 "쿨링 스프레이" 써있으면 → "쿨링 스프레이"
  예: "Batiste Dry Shampoo" 써있으면 → "바티스트 드라이샴푸"
  예: "만능클리너" 써있으면 → "만능클리너"

샴푸 vs 트리트먼트/린스 구분: 라벨 텍스트로만 판단
  라벨에 "샴푸"/"shampoo" → 샴푸 / "트리트먼트"/"treatment"/"컨디셔너" → 트리트먼트
  라벨 안 보이면 → "려 헤어제품" (샴푸/트리트먼트 임의 판단 금지)
