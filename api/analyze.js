...
// thinking 레벨: low (비용 정상화 단계). 기본 low, 필요 시 env var로 medium 즉시 전환 가능.
//  - 품질 향상의 핵심은 "프롬프트 재설계"(회피 문구 제거 → 라벨 적극 판독)였음. thinking medium은
//    거기에 비용(건당 31원)만 더 얹은 것으로 판단 → low로 되돌려 비용을 5~17원으로 정상화.
//  - low로도 개선된 프롬프트가 품질을 방어. 테스트로 품질이 부족하면 Vercel 환경변수
//    GEMINI_THINKING_LEVEL=medium 한 줄로 즉시 복귀(코드 재배포 불필요).
//  - 5개 잘림은 medium일 때만 발생했던 문제(생각 토큰이 출력 칸 잠식). low+propertyOrdering+상한 8192로 방어됨.
...
const BASE_GEN_CONFIG = {
  ...
  maxOutputTokens: 8192,                              // medium: 생각 토큰이 늘어도 목록이 안 잘리도록 출력 칸 확보(잘림 방어). 비용은 💰 로그로 실측 관리
  thinkingConfig: { thinkingLevel: THINKING_LEVEL },  // medium: 라벨 꼼꼼히 읽기 → 인식 품질의 핵심
...
[물건 이름 작성 규칙 — 핵심: "성의 있는 이름"을 만드세요]

규칙 1 — 라벨·글자·로고를 끝까지 읽으세요 (가장 중요)
  포장지·통·병에 적힌 글자를 흐릿해도 최대한 판독하세요. 한글·영문·로고를 적극적으로 읽어 실제 제품명을 살립니다.
  (예: "RYO 트리트먼트", "만능 클리너", "now 비타민", "쿨링 스프레이", "일리윤 워시")
  전체 이름은 20자 이내로 간결하게.

규칙 2 — 글자가 안 읽히면, 한국 가정의 흔한 물건으로 합리적으로 추론하세요
  한국 가정·욕실·주방·신발장·다용도장에 흔히 있는 물건이면, 모양·맥락으로 가장 그럴듯한 이름을 적극적으로 답하세요.
  "노란색 원통형 통"(X) 같은 회피보다, 맥락상 가장 가능성 높은 물건명을 우선하세요.
  (예: 분무 헤드 달린 병 → "스프레이 세정제" / 펌프 용기 → "핸드워시 or 로션" / 둥근 캔 → 라벨 색·그림으로 "살충제·스프레이" 추정)
  색·형태 묘사는 추론이 정말 불가능할 때만 쓰는 "최후의 수단"입니다. 남발하지 마세요.

규칙 3 — 색상은 "구별이 필요할 때만" 붙이세요 (가장 자주 어기는 규칙 — 주의)
  ★ 그 종류의 물건이 화면에 1개뿐이면 색상을 붙이지 마세요. 깔끔한 이름이 원칙입니다.
    (O) "무선 마우스" "탁상용 선풍기" "스마트폰" "헤드램프"
    (X) "빨간색 무선 마우스" "회색 탁상용 선풍기" "검은색 스마트폰"  ← 1개뿐인데 색을 붙임(군더더기)
  ★ 같은 종류가 2개 이상 있어서 서로 구별해야 할 때만, 색상·위치·브랜드로 구별하세요.
    (예: 우산 3개 → "검정 우산", "흰 우산", "보라 우산" / 옷 여러 벌 → "블랙 스웨터", "화이트 블라우스", "NIKE 흰 반팔티")
  ★ 단, 색상 자체가 그 물건의 핵심 정체성이면 1개여도 붙여도 됩니다. (예: "노란 고무장갑")

규칙 4 — 옷·천류·묶음류는 개수 파악
  같은 물건이 여러 개 겹쳐 있으면 보이는 만큼 개수(qty)에 반영. (예: 물티슈 4팩 → qty 4)

[환각만 차단 — 아래 "위험한 추측"만 금지. 그 외에는 위 규칙대로 적극적으로 이름 지으세요]
- 공구류(커터칼·가위·칼·드라이버 등)는 날·칼날 형태가 명확히 보일 때만 그 이름 사용. 막연히 긴 물건을 "커터칼"로 단정 금지.
- 물티슈 캡·그림자·반사광을 스마트폰·리모컨 등 전자기기로 착각하지 말 것.
- 수납장 문·선반·벽·바닥·옷걸이·배경 자체는 물건으로 세지 말 것.
- ★ "정리·수납 대상이 아닌 것"은 제외하세요: 벽에 붙은 전등 스위치·콘센트·인터폰, 멀리 뒤편 다른 방·복도·주방에 있는 큰 가전(냉장고·세탁기 등)이나 가구. 사진에 우연히 걸린 배경 공간의 물건은 세지 마세요. (예: 책상을 찍었는데 뒤쪽 주방 냉장고가 보인다고 "냉장고"를 넣지 말 것)
  → 지금 정리하려는 그 공간(책상 위·수납장 칸 안 등) 안에 실제로 놓인 물건만 대상으로 합니다.
(주의: 이 차단은 "위험한 오인"만 막기 위함입니다. 일반 생활용품의 이름을 정하는 것을 주저하게 만들면 안 됩니다 — 흔한 물건은 자신 있게 이름을 붙이세요.)
