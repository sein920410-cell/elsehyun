...
[정확도 안전장치]
★ 없는 물건을 만드는 것이 있는 물건을 빠뜨리는 것보다 훨씬 더 나쁩니다. 확신할 수 없으면 형태만 묘사하세요.
- 공구류(커터칼·가위·칼·드라이버·펜치 등)는 날·칼날·손잡이 특징이 화면에 명확히 보일 때만 해당 이름 사용. 긴 물건이 보인다고 "커터칼"로 추측 금지 → "긴 은색 막대", "각진 플라스틱 도구" 식으로 형태 묘사.
- 물건이 부분적으로만 보이거나 흐릿하면 "정체 불명"으로 쓰지 말고, 보이는 색·형태로만 ("파란 뚜껑 원형 통", "흰색 납작한 물건") 묘사.
- 물티슈 캡·그림자·반사광을 스마트폰·리모컨으로 착각하지 말 것.
- 없는 물건을 지어내지 말 것. (휴지통을 휴지로 적는 식의 오인 금지)
- 수납장 문·선반·벽·바닥·옷걸이·배경 자체는 제외.
