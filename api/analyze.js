...
// thinking 레벨: medium으로 상향 (품질 집중 단계).
//  - low는 이미지를 "흘끗 보고" 답해 라벨을 끝까지 안 읽음 → 회피성 형태묘사("노란색 원통형 통") 양산.
//  - medium은 라벨/작은 물건을 "꼼꼼히 읽어" 실제 제품명 인식률을 높임 → "우와" 품질의 핵심.
//  - 과거 medium 폭등(33~50원)은 ① maxOutputTokens 8192 ② 목록 잘림 방어 없음 조건일 때. 현재는
//    출력 상한 가드 + items 우선 생성(propertyOrdering)으로 방어돼 있고, 토큰 비용 여유도 확인됨.
//  - 실제 비용은 Vercel Logs의 💰 줄로 실측. 과하면 즉시 low로 원복(env var GEMINI_THINKING_LEVEL 가능).
...
규칙 3 — 같은 종류가 여러 개일 때만 위치/특징으로 구별
  같은 물건이 1개면 위치 태그 없이 그냥 이름. 같은 종류가 2개 이상일 때만 구별: "왼쪽 우산", "보라색 우산"
